...
[[https://www.quicklisp.org/beta/releases.html][libraries]] with a few simple commands.[fn:1]  It works with:
...
Pathnames and namestrings[fn:2] are two kinds of objects that
...
- pathname namestring[fn:2] (denoting the corresponding pathname)
...
#+begin_defun
Returns =true= if stream is an interactive stream[fn:5]; otherwise, returns
=false=.
#+end_defun
...
book[fn:3] does emphasize a  side-effect-free, applicative approach to
...
[fn:5]A stream  on which it  makes sense  to perform interactive  querying. The
...
[fn:2]namestring:  a  string  that  represents  a  filename  using  either  the
standardized    notation    for    naming    logical    pathnames    or    some
implementation-defined notation for naming a physical pathname.

[fn:1]The libraries were last updated on July 15, 2020.

...
** Commoin Lisp The Language 2nd Edition
 ...
 #+begin_src emacs-lisp
   (defun create-new-project (project title author &optional bucket)
	 ...
	 (let* ((cur-buf (current-buffer))
		...
		(delete-hl-list (list "build-scripts" "README")))
	   (message "Deleting %s..." delete-hl-list)
	   (seq-do (lambda (cid)
		     (message "...Deleting %s in %s..." cid cur-buf)
		     (let* ((beg-end (find-hl proj-tree cid))
			    (beg (car beg-end))
			    (end (cdr beg-end)))
...
#+begin_src emacs-lisp
  ...
  (defun add-s3-bucket (s3-bucket)
     "Add the name of the associated AWS S3 bucket to an Org
  templated file in the PWD."
     (with-current-buffer (car (find-file-noselect "*.org" nil nil t))
       (goto-char (point-min))
	 ...
	 (unless (re-search-forward "^#[+]bucket:" hl1 t)
	   ;; If no bucket keyword is found, search for a keyword MACRO with the value 'version'
	   (message "No bucket found; adding %s..." s3-bucket)
	   (re-search-forward "^#[+]macro:version")
	   (beginning-of-line 2)
	   (insert (concat "#+bucket:" s3-bucket))
	   (message "Added bucket %s at point %s" s3-bucket (line-beginning-position))
	   (newline)
	   (save-buffer)))))
#+end_src
**** Add a Custom_Id
 ...
 #+begin_src emacs-lisp
    ...
    drawer (creating one if necessary) at headline HL."
	 ...
	 (re-search-forward (concat "^*\\{1,\\}\s*" hl))
	 (beginning-of-line 2)
...
template file.
 ...
 #+begin_src emacs-lisp
   ...
   ;(message (format "Checking for s3-bucket: %s..." (org-template-bucket)))

...
#+author:LOLH
#+date:2021-11-22 06:49
#+macro:version Version 0.0.111
...
#+cindex:loop clause
When Lisp encounters a *loop macro call form*, it invokes the Loop Facility and
passes to it the  /loop clauses/ as a list of /unevaluated  forms/, as with any
macro.

#+texinfo:@subheading Loop Clauses
#+cindex:loop keyword
The *loop  clauses* contain Common  Lisp forms  and /loop keywords/.   The loop
keywords are recognized  by their symbol name, regardless of  the packages that
contain them.  The  loop macro translates the given form  into Common Lisp code
...
The variables established in the loop construct  are bound as if by using ~let~
or ~lambda~.  Implementations can interleave the setting of initial values with
the  bindings.   However,  the  assignment  of the  initial  values  is  always
calculated in  the order specified by  the user.  A variable  is thus sometimes
bound to  a harmless  value of  the correct data  type, and  then later  in the
prologue it is set to the true initial value by using ~setq~.

#+texinfo:@subheading Parts of the Expanded Form
#+texinfo:@subheading Expanded Form
The expanded form consists of three basic parts in the =tagbody=:
#+cindex:loop prologue
#+cindex:loop body
#+cindex:loop epilogue

- Loop Prologue  ::  contains forms that are executed  before iteration begins,
...
#+cindex:@code{nil} Block---@code{return}---@code{return-from}
Expansion of the /loop macro/ produces an *implicit block* (named =nil=). Thus,
the Common Lisp  macro ~return~ and the special form  ~return-from~ can be used

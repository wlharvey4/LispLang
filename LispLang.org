...
- ~scheme-mode~     ::    a major mode distributed with Emacs; it is a modified
  form of  ~lisp-mode~ (whose  parent mode  is ~prog-mode~)  and uses  the same
  source-code editing commands  as ~lisp-mode~; it is started  with the command
  =M-x scheme-mode=; it runs the ~scheme-mode-hook~ during initialization.
  ...
  Check out the ~scheme~ customization group.


- ~cmuscheme-mode~ ::  this is  an old major  mode extension  of ~scheme-mode~;
  adds functions for code execution directly from within Emacs buffers;
...
- ~xscheme~ ::  run MIT Scheme under  Emacs; a major mode  for interacting with
  MIT Scheme.
...
*** Scheme Mode
 - Emacs comes with  two Scheme modes. Most  people prefer the one  that is not
   the  default. To  switch to  that mode,  put something  like this  into your
   ~.emacs~ file:
